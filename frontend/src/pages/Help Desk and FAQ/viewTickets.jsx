...
const ViewTickets = () => {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      setLoading(true);
      try {
        const response = await axios.get("http://localhost:5555/tickets");
  ...
  const handleDelete = async () => {
    if (ticketToDelete) {
      try {
        await axios.delete(`http://localhost:5555/tickets/${ticketToDelete}`);
        setTickets((prev) =>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 flex flex-col items-center relative">
      <div className="absolute top-0 left-0 p-6 flex items-center space-x-4">
        <h1 className="text-2xl font-semibold text-gray-800">Manage Tickets</h1>
      </div>
