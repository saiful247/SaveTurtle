...
const EventProgramPage = () => {
  ...
  const downloadPDF = () => {
    ...
    const tableColumn = ['No.', 'Event Name', 'Venue', 'Date', 'Time', 'Price'];
    const tableRows = [];
  ...
  return (
    <div className='p-4'>
      ...
      <div className="my-4">
        <h3 className="text-lg text-gray-600">Total Events: {filteredEvents.length}</h3>
      </div>
      ...
      {loading ? (
        ...
        <table className='w-full border-collapse'> {/* Change to border-collapse for outlines */}
  <thead className='bg-gray-200'> {/* Added light gray background */}
    <tr>
      <th className='border border-slate-600 p-2 text-left'></th>
      <th className='border border-slate-600 p-2 text-left max-md:hidden'>Event Name</th>
      <th className='border border-slate-600 p-2 text-left max-md:hidden'>Venue</th> {/* Fixed typo: "Vanue" to "Venue" */}
      ...
      <th className='border border-slate-600 p-2 text-left'>Operations</th>
    </tr>
  </thead>
  <tbody>
    {filteredEvents.map((eventP, index) => (
      <tr key={eventP._id} className='h-8 even:bg-gray-100'> {/* Added alternating row color */}
        <td className='border border-slate-700 text-center p-2'>{index + 1}</td>
        <td className='border border-slate-700 text-center p-2'>{eventP.eventName}</td>
        <td className='border border-slate-700 text-center p-2 max-md:hidden'>{eventP.vanue}</td>
        <td className='border border-slate-700 text-center p-2'>{eventP.date}</td>
        <td className='border border-slate-700 text-center p-2'>{eventP.time}</td>
        <td className='border border-slate-700 text-center p-2'>{eventP.price}</td>
