...
const ViewAvailablePacks = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5555/memberships')
      .then(response => {
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {filteredMemberships.map(membership => (
            <div key={membership._id} className="bg-white shadow-lg rounded-lg p-6">
              <h2 className="text-3xl font-bold text-green-700">{membership.name}</h2>
              <p className="text-green-600 mt-2">Price: Rs {membership.price}</p>
              <p className="text-green-600 mt-2">Duration: {membership.duration} Months</p>
